...
Fy = np.zeros(shape=(ny, nx))
Ay = -tau_y * np.ones_like(Fy)
By = 1.0 + 2.0 * tau_y * np.ones_like(Fy)
Cy = -tau_y * np.ones_like(Fy)
...
Fx = np.zeros(shape=(nx, ny))
Ax = -tau / (hx * hx) * np.ones_like(Fx)
Bx = 1.0 + 2.0 * tau / (hx * hx) * np.ones_like(Fx)
Cx = -tau / (hx * hx) * np.ones_like(Fx)
...
while err > 4.0 * h0**2:
    ...
    Fy[1:-1, 1:-1] = (inner(u) + tau_x * diff_X2(u) + tau * inner(F)).T
    uh = progon(Ay, By, Cy, Fy).T

    # Прогонка в другую сторону
    Fx[1:-1, 1:-1] = inner(uh) + tau_y * diff_Y2(uh) + tau * inner(F)
    un = progon(Ax, Bx, Cx, Fx)
